...
const LAYERS = [
  { fill: POP.purpleDeep, floor: 0.80, base: 0.13, span: 0.15, width: 0.150, speed: 0.010, seed: 3, lit: false },
  { fill: POP.teal, floor: 0.89, base: 0.17, span: 0.19, width: 0.190, speed: 0.021, seed: 61, lit: false },
  { fill: POP.black, floor: 0.99, base: 0.23, span: 0.23, width: 0.245, speed: 0.038, seed: 137, lit: true },
];
...
registerTrack({
  ...
  wedges: [[6, 96, POP.purple], [148, 198, POP.teal], [252, 302, POP.purple]],
  label: { art: 'skyline', paper: POP.purple },
  ...
  render({ context: g, width: W, height: H, time, beat, pulse, pointer }) {
    ...
    for (let i = 0; i < 46; i += 1) {
      const twinkle = rand(i * 3 + Math.floor(beat + i * 0.37));
      ...
      for (let i = -1; i < columns; i += 1) {
        ...
        for (let c = 0; c < cols; c += 1) {
          for (let r = 0; r < rows; r += 1) {
            ...
            if (!on) continue;
            g.fillStyle = rand(key) > 0.82 ? POP.teal : POP.yellow;
            g.fillRect(
              x + ww * (1 + c * 2),
