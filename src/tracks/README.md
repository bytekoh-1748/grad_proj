...
새 곡의 `registerTrack` 객체 안에 필요한 시간대만 지정할 수 있습니다.
`palette`는 해당 스타일의 0–2번 팔레트입니다. 생략한 시간대는 기존 곡의 기본값 또는
홈 시간대 분위기를 사용하므로 새 곡을 위해 공용 렌더러를 수정할 필요가 없습니다.

```js
moods: {
  day: { edition: 'expressive', palette: 2 },
  night: { edition: 'liquid', palette: 0 },
},
```

LP 개수에는 6장 제한이 없습니다. 선반에는 최대 5장만 펼치고 이전·다음 버튼으로
모든 곡에 접근합니다. `duration`은 양수인 `분:초` 형식이며 초는 00–59입니다.
실제 음원 재생은 아직 연결되어 있지 않고, 이 계약은 LP의 시각적 재생 장면을 정의합니다.
