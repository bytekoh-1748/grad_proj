...
export function drawVariant(kind,variant,{rect,text,mono,line,ellipse,ring,polygon,rounded,path,centered,clip,hole,H,accent,date,content,appearance={}}){
  ...
  if(kind==='calendar'&&variant==='orbit'){
    ...
    polygon([[791,31],[788,205],[942,193]],accent,.76);line(100,126,755,126,INK,5,.81);mono('06 / PASSING DAYS',115,106,28,INK,.86);rect(100,155,621,118,accent,.7);mono(month+' / '+date.getFullYear(),117,238,63,INK,.87);centered(day,509,828,500,INK,.93,750);text(date.toLocaleDateString('en-GB',{weekday:'short'}).toUpperCase(),119,H-193,82,INK,.87,650);
    ...
    ellipse(500,464,440,440,INK,.56);for(let r=424;r>163;r-=8)ring(500,464,r,r%3?'#555547':'#929482',1.7,.62);ellipse(500,464,153,153,accent,.78);centered(appearance.text||'SIDE A',500,407,52,INK,.88,700);centered('33⅓',500,559,34,INK,.85,500);hole(500,464,18);
    ...
    const points=Array.from({length:48},(_,i)=>{const a=i*Math.PI/24,r=i%2?357:478;return[500+Math.cos(a)*r,500+Math.sin(a)*r];});polygon(points,accent,.65);ellipse(500,500,324,324,INK,.58);ellipse(500,500,293,293,'#ff7353',.65);ellipse(500,500,229,229,PAPER,.72);ellipse(500,500,168,168,accent,.82);ellipse(500,500,99,99,BLUE,.89);if(appearance.text)centered(appearance.text,500,810,35,INK,.92,650);
    ...
    centered(appearance.text||'SOFT WEATHER',500,439,74,INK,.89,700);centered('A CLOUD OF YOUR OWN',500,505,25,INK,.79,500);
  }else if(kind==='book'){
    ...
    polygon([[835,34],[817,209],[961,181]],accent,.77);displayWords(appearance,'KEEP WHAT MOVES YOU.').slice(0,4).map((word,i)=>i===3?displayWords(appearance,'KEEP WHAT MOVES YOU.').slice(3).join(' '):word).forEach((word,i)=>text(word,116,331+i*202,166,INK,.9,650));
  }else if(kind==='camera'){
    disk(470,PAPER);disk(451,INK);clip(ctx=>ctx.arc(500,500,430,0,Math.PI*2),()=>{rect(60,60,880,880,BLUE,.35);for(let i=0;i<22;i++)line(500,500,75+i*42,953,'#c1b4ee',2,.42);ellipse(611,401,243,243,'#b19cdf',.58);for(let y=181;y<830;y+=14)line(80,y,930,y+120,PAPER,1,.63);centered(displayWords(appearance,'AFTER IMAGE')[0],497,474,179,PAPER,.92,850);centered(displayWords(appearance,'AFTER IMAGE').slice(1).join(' '),499,642,179,PAPER,.91,850);});
    centered('AFTERIMAGE.PNG',500,799,28,accent,.86,600);
  }else if(kind==='cassette'){
    rounded(174,111,652,366,180,INK,.48);[258,741].forEach(x=>{ellipse(x,294,230,230,accent,.6);ring(x,294,195,INK,11,.7);ring(x,294,146,INK,5,.74);ellipse(x,294,85,85,INK,.61);for(let i=0;i<6;i++){const a=i*Math.PI/3;line(x+30*Math.cos(a),294+30*Math.sin(a),x+63*Math.cos(a),294+63*Math.sin(a),PAPER,15,.89);}hole(x,294,15);});line(258,71,741,71,INK,6,.76);line(258,517,741,517,INK,6,.76);rounded(264,558,473,82,41,PAPER,.7);centered(appearance.text||'IN BETWEEN',500,614,42,INK,.89,750);
  }else if(kind==='calculator'){
    rounded(51,24,898,H-50,[449,449,76,76],PAPER,.61);rounded(157,192,686,239,60,INK,.42);centered(appearance.value||'128.00',501,353,141,accent,.88,650);
    ...
    rounded(27,25,946,573,75,INK,.6);polygon([[122,547],[81,H-31],[303,584]],INK,.6);mono('room@somewhere',82,115,30,accent,.82);line(83,151,909,151,'#566149',2,.65);mono('> ls /unfinished',82,245,45,accent,.9);mono('dreams    tomorrow',82,323,38,PAPER,.85);text(appearance.text||'STILL HERE_',82,476,113,PAPER,.94,750);rect(83,516,27,39,accent,.85);
  }else if(kind==='radio'){
    rounded(22,21,956,H-42,(H-42)/2,accent,.62);text(appearance.value||'88.7',102,265,170,INK,.92,650);mono('FM',553,259,42,INK,.86);ellipse(794,H/2,139,139,INK,.54);for(let r=127;r>49;r-=10)ring(794,H/2,r,'#87634b',2,.69);ellipse(794,H/2,36,36,PAPER,.84);line(794,H/2,814,H/2-25,INK,6,.92);for(let i=0;i<32;i++)line(124+i*15,300,124+i*15,i%4?321:335,INK,2,.78);mono('ROOM.FM',162,114,28,INK,.8);
  }else if(kind==='portal'){
    ...
    centered(displayWords(appearance,'ELSE WHERE')[0],500,770,189,PAPER,.94,750);centered(displayWords(appearance,'ELSE WHERE').slice(1).join(' '),500,950,189,PAPER,.94,750);centered('↗',500,1267,257,PAPER,.92,500);
  }else if(kind==='sketch'){
    ...
    rect(95,61,810,H-158,BLUE,.35);for(let i=0;i<17;i++)polygon([[125+i*45,64],[151+i*45,64],[111+i*45,H-100],[73+i*45,H-100]],i%3?BLUE:'#b4a0df',.48+i%3*.08);centered(appearance.text||'MOVEMENT',500,257,117,PAPER,.94,850);polygon([[110,H-67],[110,H-38],[135,H-52]],INK,.89);line(179,H-52,866,H-52,INK,2,.79);rect(179,H-54,266,5,accent,.9);
  }else if(kind==='scope'){
    disk(474,PAPER);disk(455,INK);clip(ctx=>ctx.arc(500,500,432,0,Math.PI*2),()=>{for(let i=0;i<19;i++){line(60+i*49,60,60+i*49,940,'#385447',1,.3);line(60,60+i*49,940,60+i*49,'#385447',1,.3);}for(let r=95;r<420;r+=100)ring(500,500,r,'#819c66',2,.61);polygon([[500,500],[500,69],[719,130]],accent,.64);line(500,500,719,130,'#f16d48',9,.89);[[341,642],[622,327],[701,642]].forEach(([x,y])=>{ellipse(x,y,12,12,accent,.94);ring(x,y,29,accent,2,.8);});});centered(appearance.text||'SIGNAL',500,854,62,PAPER,.84,600);
