...
function warpArtwork(shapes,h,angle=0,lift=0) {
  ...
  shapes.forEach(shape=>{
    ...
    shape.node.setAttribute('stroke-width',shape.width*scale);
  });
}
...
export class VectorRoom {
  constructor({stage,billboard,tracks,reduced,onSelect,onView,onGesture}) {
    ...
    this.room=stage.parentElement;this.selected=0;this.view='home';this.playing=false;this.activity='home';this.musicAmount=0;
    ...
    this.lightHour=this.lightTarget=13;this.lightDirection=1;this.timeAuto=true;
    this.artPlane=document.getElementById('track-plane');this.artContext=new SVGContext(document.getElementById('track-drawing'));
    this.deckShadow=svg('path',{fill:'var(--ink)',opacity:'.23'});stage.querySelector('#record-shadows').append(this.deckShadow);
    ...
    this.armShadowShapes.forEach(shape=>{shape.node.setAttribute('fill','var(--ink)');shape.node.setAttribute('stroke','var(--ink)');});
    ...
    this.cards=tracks.map((track,index)=>{
      ...
      const edge=svg('path',{fill:'#302737',stroke:'#09090b','stroke-width':2});depth.append(edge);group.append(depth,front);layer.appendChild(group);
      ...
      const shadow=svg('path',{fill:'var(--ink)',opacity:.17});stage.querySelector('#record-shadows').appendChild(shadow);
      return{group,shadow,edge,spin:group.querySelector('.record-spin'),spinAngle:0,index,mode:index===0?'cue':'stack',pose:null,flight:null};
    });
  ...
  resize(){
    this.width=this.room.clientWidth;this.height=this.room.clientHeight;this.logicalHeight=1600*this.height/this.width;
  ...
  setView(name,user=false){
    ...
    this.view=name;this.dragOffset=[0,0];this.zoomTarget=1;this.contours.to(floorView(name));
  ...
  setHour(hour){this.lightTarget=Math.max(7,Math.min(19,hour));this.timeAuto=false;this.onGesture?.();this.updateTimeUI();}
  updateTimeUI(){
    const total=Math.round(this.lightTarget*60),value=`${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`;
    document.getElementById('light-readout').textContent=value;
    const range=document.getElementById('light-time');if(document.activeElement!==range)range.value=this.lightTarget;
    range.setAttribute('aria-valuetext',`장면 속 빛의 시간 ${value}`);
    document.getElementById('light-auto').setAttribute('aria-pressed',String(this.timeAuto));
  }
  bind(){
    const options={signal:this.abort.signal};
    this.room.addEventListener('pointerdown',e=>{
      if(e.button!==0||e.target.closest('button,input,video,dialog,#billboard,.room-controls'))return;
    ...
    this.room.addEventListener('wheel',e=>{if(e.target.closest('input,video,dialog'))return;e.preventDefault();this.zoomBy(-e.deltaY*.0004);},{...options,passive:false});
    document.getElementById('light-time').addEventListener('input',e=>this.setHour(Number(e.target.value)),options);
    document.getElementById('light-auto').addEventListener('click',()=>{this.timeAuto=!this.timeAuto;this.updateTimeUI();},options);
  }
  pointAt(event){
  ...
  changeMode(card,mode){
    ...
    return new Promise(resolve=>{card.flight={from:card.pose?{...card.pose}:null,start:performance.now(),duration:this.reduced?0:1100,resolve};});
  }
  select(index){
    this.selected=index;
    this.cards.forEach(card=>{card.group.setAttribute('aria-pressed',String(card.index===index));if(card.index!==index)this.changeMode(card,'stack');});
  ...
  paintBackground(frame){
    ...
    this.stage.querySelector('#floor').setAttribute('d',closed(ground));
    this.stage.querySelector('#floor-clip-path').setAttribute('d',closed(ground));
    this.stage.querySelector('#seam').setAttribute('d',pathText([[-2500,edge(-2500)],[4100,edge(4100)]]));
    for(const[i,lo,hi]of[[0,0,28],[1,28,68],[2,68,112]]){
      this.stage.querySelector(`#boundary-${i}`).setAttribute('d',closed(mapPoints(wall,[[-1200,lo],[2700,lo],[2700,hi],[-1200,hi]])));
    }
    this.stage.querySelector('#floor-hatching').setAttribute('d',closed(mapPoints(floor,[[-1200,1],[2700,1],[2700,23],[-1200,23]])));
    this.stage.querySelector('#seam-inner').setAttribute('d',pathText(mapPoints(floor,[[-1200,8],[2700,8]])));
    const degrees=Math.atan2(b[1]-a[1],b[0]-a[0])*180/Math.PI;
    this.stage.querySelector('#hatch').setAttribute('patternTransform',`rotate(${degrees})`);
  }
  paintLight(frame){
    const patches=lightPatches(frame.floor,frame.wall,this.lightHour);
    patches.forEach((p,i)=>{
      this.stage.querySelector(`#floor-light-${i}`).setAttribute('d',closed(p.floor));
      this.stage.querySelector(`#wall-light-${i}`).setAttribute('d',closed(p.wall));
    });
    const light=daylight(this.lightHour);
    this.stage.querySelector('#floor-light').style.opacity=light.opacity;
    this.stage.querySelector('#wall-light').style.opacity=light.opacity*.74;
    return light;
  }
  ...
  render(dt,now){
    ...
    this.arm+=(this.armTarget-this.arm)*(this.reduced?1:1-Math.exp(-dt/250));this.armLift+=(this.armLiftTarget-this.armLift)*ease;
    this.artOpacity+=(this.artTarget-this.artOpacity)*(this.reduced?1:1-Math.exp(-dt/600));
    if(this.timeAuto&&!this.reduced&&!this.billboard.matches(':hover,:focus-within')){
      this.lightTarget+=this.lightDirection*dt/60000;
      if(this.lightTarget>=19||this.lightTarget<=7){this.lightTarget=Math.max(7,Math.min(19,this.lightTarget));this.lightDirection*=-1;}
    }
    this.lightHour+=(this.lightTarget-this.lightHour)*(this.reduced?1:1-Math.exp(-dt/520));
    const px=this.pointer.x,py=this.pointer.y;
    this.pointer.x+=(this.pointerTarget.x-px)*ease;this.pointer.y+=(this.pointerTarget.y-py)*ease;
    this.pointer.dx+=(this.pointer.down?(this.pointer.x-px)*20:-this.pointer.dx)*ease;
    this.pointer.dy+=(this.pointer.down?(this.pointer.y-py)*20:-this.pointer.dy)*ease;
    ...
    const deckMoving=Math.abs(this.musicAmount-(this.lastMusicAmount??-1))>.00001;this.lastMusicAmount=this.musicAmount;
    const zoom=[this.zoom,0,0,this.zoom,800*(1-this.zoom),this.logicalHeight*.5*(1-this.zoom)];
    this.stage.querySelector('#drawing').setAttribute('transform',matrixText(zoom));
    document.getElementById('arm-drawing').setAttribute('transform',matrixText(zoom));
    if(moving||this.needsPaint)this.paintBackground(frame);
    if(moving||this.needsPaint||deckMoving)warpArtwork(this.baseShapes,h);
    ...
    for(const id of ['deck-base','deck-sides'])this.stage.querySelector('#'+id).style.opacity=entry.opacity;
    document.getElementById('deck-arm').style.opacity=entry.opacity;
    document.getElementById('arm-shadow').style.opacity=entry.opacity*.18;
    if(moving||this.needsPaint||deckMoving||Math.abs(this.arm-(this.drawnArm??Infinity))>.0008||Math.abs(this.armLift-(this.drawnLift??Infinity))>.001){
      const armH=homography(mapPoints(raisedFloor(frame,65+entry.z),deckPoints).flat(),760,560);
      warpArtwork(this.armShapes,armH,this.arm,this.armLift);warpArtwork(this.armShadowShapes,h,this.arm,this.armLift*.28);this.drawnArm=this.arm;this.drawnLift=this.armLift;
    }
    const light=this.paintLight(frame);
    ...
    this.cards.forEach(card=>{
      ...
      else{target={...OBJECTS.stack[slot],z:12+slot*10};rank=slot+1;slot++;}
      let pose=target;
      if(card.flight){
        const f=card.flight,t=f.duration?Math.min(1,(now-f.start)/f.duration):1;
      ...
      card.pose=pose;card.group.style.zIndex=rank;
      ...
      if(this.playing&&card.index===this.selected&&!this.reduced){card.spinAngle=(card.spinAngle+dt*.2)%360;card.spin.setAttribute('transform',`rotate(${card.spinAngle} 100 100)`);}
    });
    this.activities?.render(frame);
